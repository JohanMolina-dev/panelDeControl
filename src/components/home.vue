<template>
  <div>
    <div class="jumbotron jumbotron-fluid">
      <div class="container">
        <h1 class="display-4">Principales Funciones</h1>
        <p class="lead">Software para Gestión de Proveedores</p>
      </div>
    </div>
    <div class="container">
      <div class="row">
        <div class="col-4">
          <div class="list-group" id="list-tab" role="tablist">
            <a
              class="list-group-item list-group-item-action active"
              id="list-home-list"
              data-toggle="list"
              href="#list-home"
              role="tab"
              aria-controls="home"
              >Requerimientos Internos</a
            >
            <a
              class="list-group-item list-group-item-action"
              id="list-profile-list"
              data-toggle="list"
              href="#list-profile"
              role="tab"
              aria-controls="profile"
              >Portal de Proveedores</a
            >
            <a
              class="list-group-item list-group-item-action"
              id="list-messages-list"
              data-toggle="list"
              href="#list-messages"
              role="tab"
              aria-controls="messages"
              >Compare sus proveedores</a
            >
            <a
              class="list-group-item list-group-item-action"
              id="list-settings-list"
              data-toggle="list"
              href="#list-settings"
              role="tab"
              aria-controls="settings"
              >Gestion de contratos</a
            >
          </div>
        </div>
        <div class="col-8">
          <div class="tab-content" id="nav-tabContent">
            <div
              class="tab-pane fade show active"
              id="list-home"
              role="tabpanel"
              aria-labelledby="list-home-list"
            >
              <p>
                Usted podrá consolidar todas las solicitudes de adquisición de
                su empresa en una sola aplicación y mantener una comunicación
                efectiva con todos los involucrados del ciclo de gasto, mediante
                notificaciones personalizadas. Los responsables podrán solicitar
                o aprobar órdenes de compra desde su computador o dispositivos
                móviles.
              </p>
            </div>
            <div
              class="tab-pane fade"
              id="list-profile"
              role="tabpanel"
              aria-labelledby="list-profile-list"
            >
              <p>
                Esta funcionalidad del software de compras Giitic le facilitara
                a usted y a sus proveedores el registro, reporte de
                cotizaciones, novedades, soporte y demás requerimientos que
                usted considere importante. Obtenga ventajas competitivas al
                fortalecer las relaciones estratégicas con sus partners.
              </p>
            </div>
            <div
              class="tab-pane fade"
              id="list-messages"
              role="tabpanel"
              aria-labelledby="list-messages-list"
            >
              Mediante la generación automática de un cuadro comparativo e
              informes personalizados, usted podrá evaluar y calificar a sus
              proveedores. Su empresa se destacará por tomar decisiones
              acertadas. Elija siempre al mejor proveedor y obtenga grandes
              beneficios para su organización
            </div>
            <div
              class="tab-pane fade"
              id="list-settings"
              role="tabpanel"
              aria-labelledby="list-settings-list"
            >
             <p> Genere y controle contratos que maneja con sus proveedores, reciba
              alertas personalizadas de vencimientos y renovaciones contratadas
              desde el módulo de gestión de compras. Las alertas personalizadas
              le permitirán tener control y supervisión de los contratos que
              tiene con sus proveedores.</p>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>


<script>
import axios from "axios";
export default {
  data() {
    return {
      datos: null,
      team: [
        {
          codigo: 1,
          nombre: "Lucas Melo",
          descripcion:
            "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Mauris condimentum ac elit et accumsan.",
          rol: "Desarrollador backend",
          image:
            "data:image/png;base64,",
        },
        {
          codigo: 2,
          Nombre: "Cristian Sanz",
          descripcion:
            "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Mauris condimentum ac elit et accumsan.",
          rol: "Arquictecto de software",
          image:
            "data:image/png;base64,",
        },
        {
          codigo: 3,
          nombre: "Samara Cerezo",
          descripcion:
            "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Mauris condimentum ac elit et accumsan.",
          rol: "Tester",
          image:
            "data:image/png;base64,",
        },
        {
          codigo: 4,
          nombre: "Iris Mesa",
          descripcion:
            "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Mauris condimentum ac elit et accumsan.",
          rol: "Analista",
          image:
            "data:image/png;base64,",
        },
      ],
    };
  },
  mounted() {
    axios.get("articulo/list").then((response) => (this.datos = response.data));
  },
};
</script>